...
IF NOT EXISTS (SELECT 1 FROM Customer)
    INSERT INTO Customer (Name, PhoneNumber, Email, Address) VALUES
        (N'ООО "АгроМаркет"', '+7 495 123-45-67', 'info@agromarket.ru', N'г. Москва, ул. Ленина, 10'),
        (N'ООО "ЗерноТорг"', '+7 499 345-67-89', 'info@zernotorg.ru', N'г. Москва, пр. Мира, 22');
GO
...
BEGIN
    ...
    IF @Emp IS NOT NULL
        INSERT INTO SalesOrder (CustomerID, EmployeeID, StockID, StatusID, CreationDate, PricePerKg) VALUES
            (1, @Emp, 1, 1, '2026-06-09', 5.00),
            (2, @Emp, 2, 2, '2026-06-08', 4.00);
END
GO
